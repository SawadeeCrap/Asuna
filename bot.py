...
for name, token in [("TELEGRAM_TOKEN", TELEGRAM_TOKEN),
    ...
    if not token:
        raise ValueError(f"{name} не установлен!")
...
def embed_text(text: str) -> list:
    """Получение эмбеддинга через OpenRouter с логированием ошибок"""
    if not text.strip():
        return [0.0] * 1536
    try:
        resp = requests.post(
        ...
        logger.error(f"Ошибка эмбеддинга для текста '{text}': {e} | Ответ: {getattr(resp, 'text', 'нет ответа')}")
        return [0.0] * 1536
...
def handle_message(message):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Ошибка парсинга ответа OpenRouter: {response.text}")
