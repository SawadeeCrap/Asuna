...
Всегда отвечай полезно и подробно."""
        ...
        if user_context:
            for msg in user_context[-10:]:  # последние 6 сообщений для контекста
                messages.append(msg)
        ...
        response = requests.post(
            ...
            json={
                ...
                "messages": messages,
                "temperature": 0.6,  # Повышена для более творческих ответов
                "max_tokens": 400    # Увеличено для более подробных ответов
            }
        )
...
def handle_start(message):
    ...
    if is_admin(user_id):
        welcome_text = """🤖 Привет, Админ! Я Asuna, кит компаньен.
...
• НЕ могут изменять общую базу знаний"""
        ...
        welcome_text = f"""🤖 Привет, {username}! Я Asuna, кит компаньен.
...
ℹ️ Примечание: Общую базу знаний может пополнять только администратор."""
