...
> **Самый короткий путь:** двойной клик по `Myrmex.command`. Первый раз он всё установит,
> дальше запускай **Myrmex.app**. Все настройки ниже есть в окне приложения. Эта
> инструкция объясняет, что за ними стоит, и описывает запуск из терминала.
